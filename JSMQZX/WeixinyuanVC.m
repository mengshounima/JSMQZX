...
#import "WeixinyuanVC.h"
#import "JKAlertDialog.h"
#import "XinyuanCellFrame.h"
@interface WeixinyuanVC ()<UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate>
{
    JKAlertDialog *alert;
...
-(void)getViewData{
    ...
    [[HttpClient httpClient] requestWithPath:@"/GetWXYInfo" method:TBHttpRequestPost parameters:paramFuwu prepareExecute:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        for (NSDictionary *dict in SearchShowArr) {
            ...
            XinyuanCellFrame *statusFrame = [[XinyuanCellFrame alloc] init];
            statusFrame.contentStr = [NSString stringWithFormat:@"心愿内容:%@",[dict objectForKey:@"wxy_nr"]] ;
...
{//搜索网格名
    ...
    for (int i=0;i<[LogArr count];i++) {
        ...
        //公告标题搜索
        NSString *namePinyinStr = [Single objectForKey:@"wxy_nr"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    else if (tableView == _CUNTable)
    {
        ...
        //服务列表
        static NSString *ID = @"XinyuanCell";
        XinyuanCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
        if (cell == nil) {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"XinyuanCell" owner:nil options:nil] lastObject];
        }
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView == _LogTableView) {
        // 取出这行对应的frame模型
        XinyuanCellFrame *statusFrame = _statusFrameArray[indexPath.row];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    XinyuanInfoVC *infoVC = segue.destinationViewController;
